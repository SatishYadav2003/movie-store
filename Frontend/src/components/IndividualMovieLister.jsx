    ...
    <>
      ...
      <div className="relative flex flex-col justify-between select-none bg-black min-h-screen text-white">
        ...
        <div className="w-full mb-4 sm:text-lg select-none bg-zinc-900 shadow-md rounded-md px-2 py-4">
          {/* Movie Details */}
          {[
            ["Name", movie["Movie Name"], "text-pink-400"],
            ["Description", movie["Movie Description"], "text-fuchsia-400"],
            ["Category", movie["Movie Category"], "text-red-400"],
            ["Genre", movie.Genre, "text-pink-400"],
            ["Release Date", movie["Release Date"], "text-orange-400"],
            ["Starring", movie.Staring?.trim().replace(/,\s*$/, ""), "text-green-400"],
            [
              "Director",
              movie["Director:"].slice(0, -1).length === 0
                ? "Not Known"
            ...
            ["Rating", movie.Rating, "text-yellow-400"],
          ].map(([label, value, color], index) => (
            <p
              key={index}
              className={`${color} mb-4 font-sans font-semibold`}
            >
              <span className="text-gray-300 font-bold">{label} :</span> {value}
            </p>
          ))}
        </div>
        ...
        <div className="w-full bg-sky-500 my-4 text-center py-2 text-white">
          <span className="sm:text-xl font-semibold">
            {movie["Movie Name"]} full movie download / watch
          </span>
        </div>
        ...
        <div className="w-full flex flex-wrap justify-center gap-4 mb-6">
          <motion.button
            ...
            disabled={isLoading}
            className="relative bg-sky-500 text-white font-semibold py-3 px-6 rounded-lg hover:bg-sky-700"
          >
            {isLoading ? "Loading..." : "Go to Download Section"}
          </motion.button>
          ...
          <motion.button
            ...
            className="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-3 px-6 rounded-lg"
          >
            Watch Movie
          </motion.button>
        </div>
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-xl font-semibold text-center mb-6">
              {isWatchMode ? "Watch Options" : "Choose Download Quality"}
            </h2>
            ...
            {loadingInModal ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[70vh] overflow-y-auto">
                {downloadLinks.length === 0 ? (
                  <p className="text-center w-full text-red-400 font-semibold col-span-2">
                  ...
                  downloadLinks.map((link, index) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="bg-blue-600 hover:bg-blue-700 text-white text-center font-semibold py-1.5 rounded-md"
                      >
                        {isWatchMode ? "Watch Now" : "Download"}
                      </button>
                    </motion.div>
                  ))
                )}
