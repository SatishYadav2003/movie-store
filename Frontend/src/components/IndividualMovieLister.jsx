...
function IndividualMovieLister() {
  ...
  const handleDownload = (url, headers) => {
    ...
    if (isVideoFile) {
      ...
      // Handle file download through backend API
      const encodedHeaders = encodeURIComponent(JSON.stringify(headers));
      const downloadUrl =
        `${BASE_URL}/api/download?` +
        `url=${encodeURIComponent(url)}` +
      ...
      // Hide download animation after a reasonable time (adjust as needed)
      setTimeout(() => {
        setShowDownloadAnimation(false);
      }, 5000);
    } else {
      // 2) Not a direct file—just redirect the user to that page
      // Close modal but don't show download animation
      // setShowModal(false);
      window.location.href = url;
    }
  };
      ...
      {showDownloadAnimation && (
        ...
        >
          <div className="w-64 h-64">
            <DotLottieReact
              src="https://lottie.host/edf6cc4a-5fe5-4f0a-8d0a-6708fba3a436/1lI6sW22Lw.lottie"
              loop
              autoplay
            />
          </div>
          <p className="text-white text-xl mt-4">Downloading your movie...</p>
          <button
